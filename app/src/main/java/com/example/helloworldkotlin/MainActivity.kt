...
/* Quotes from Book of Mormon: taken from
*  http://lifey.org/inspirational-book-of-mormon-scriptures/
*  Quotes from Quran: taken from
*  https://themuslimvibe.com/faith-islam/10-inspirational-verses-from-the-holy-quran-to-give-you-hope
*  Quotes from the Bible: taken from
*  https://www.biblestudytools.com/topical-verses/popular-king-james-bible-verses/
* */
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var quoteView = findViewById<TextView>(R.id.quoteView)
            ...
            // for the buttons
        val bomButton = findViewById<Button>(R.id.bomButton)
        val kjvBibleButton = findViewById<Button>(R.id.kjvBibleButton)
        val quranButton = findViewById<Button>(R.id.quranButton)

        /*
        val intro1 = "Are you in need of some inspiration? Revelation perhaps?\n " +
                    ...
                    "lead the people to do that which was just—yea, it had a more powerful\n" +
                    "effect upon the minds of the people than the sword, or anything else, " +
                    "which had happened unto them...\"\n." +
                    "We've provided for you a few religious texts to " +
                    "choose inspirational quotes from."
                    */

        var quotesBOM = db.collection("quotes-2d806").document("fI4JgLnJWr1qkfekxhIU")
            quotesBOM.get().addOnSuccessListener { documentSnapshot ->

            }

        bomButton.setOnClickListener{quotesBOM}
        quranButton.setOnClickListener { holdArrays(2) }
        kjvBibleButton.setOnClickListener { holdArrays(3) }
...
/*
    ...
    private fun holdArrays(arr: Int): Int {

        var quoteView = findViewById<TextView>(R.id.quoteView)
        val errorMessage = "Invalid selection. Here are a few quotes you might find:"
        ...
        when (arr) {
            1 -> {
                quoteView.text = listOfBoMQuotes.random()
                return 0
            }
            2 -> {
                quoteView.text = listOfQuranQuotes.random()
                return 0
            }
            3 -> {
                quoteView.text = listOfBibleQuotes.random()
                return 0
            }
            else -> {

                quoteView.text = errorMessage
